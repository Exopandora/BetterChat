import {tooltipManager} from "../util/Tooltips";
...
abstract class Attachment<T extends HTMLElement> {
    node: T
    ...
    newInstance(): HTMLElement {
        return <HTMLElement> this.node.cloneNode(true);
    }
}

class VideoAttachment extends Attachment<HTMLVideoElement> {
...
class AudioAttachment extends Attachment<HTMLAudioElement> {
...
class TwitterAttachment extends Attachment<HTMLElement> {
...
class ImageAttachment extends Attachment<HTMLElement> {
    url: string;
    naturalWidth: number;
...
class MultimediaAttachment extends Attachment<HTMLVideoElement> {
    constructor(node: HTMLVideoElement) {
        super(node);
    }
...
class GenericAttachment extends Attachment<HTMLElement> {
    url: string;
    ...
    newInstance() {
        const node = <HTMLElement> this.node.cloneNode(true);
...
function createImageEmbed(url: string, resolve: (value: Attachment<any>) => void, reject: (reason?: any) => void) {
...
function createVideoEmbed(url: string, resolve: (value: Attachment<any>) => void, reject: (reason?: any) => void) {
...
function createAudioEmbed(url: string, resolve: (value: Attachment<any>) => void, reject: (reason?: any) => void) {
...
function createMultimediaEmbed(url: string, resolve: (value: Attachment<any>) => void, reject: (reason?: any) => void) {
    const video = document.createElement("video");
...
async function createTwitterEmbed(url: string): Promise<Attachment<any>> {
...
function createGenericEmbed(url: string, html: string, resolve: (value: Attachment<any>) => void, reject: (reason?: any) => void) {
...
function createEmbed(url: URL): Promise<Attachment<any>> {
    return new Promise<Attachment<any>>((resolve: (value: Attachment<any>) => void, reject: (reason?: any) => void) => {
        fetch(url.href, {
            method: "GET",
            headers: {
...
export async function createAttachments(links: string[]): Promise<HTMLElement[]> {
    ...
    const fulfilled: HTMLElement[] = [];
    for(const [link, promise] of promises.entries()) {
        if(!attachmentCache.has(link)) {
            attachmentCache.set(link, promise);
        }
        await promise.then((attachment: Attachment<any>) => fulfilled.push(attachment.newInstance()), () => {});
    }
    return fulfilled;
}
...
function showImagePreview(url: string, naturalWidth: number, naturalHeight: number) {
    const img = document.createElement("img");
...
function truncate(string: string, size: number): string {
